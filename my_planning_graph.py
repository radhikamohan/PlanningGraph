...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        nodes = set()
        literal_level = self.s_levels[level]
        for a in self.all_actions:
            a_Node = PgNode_a(a)
            if set(a_Node.prenodes).issubset(set(literal_level)):
                nodes.add(a_Node)
                for s_Node in literal_level:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (a1_add & a2_rem) | (a2_add & a1_rem)
